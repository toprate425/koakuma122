    ...
    const getAboutUsLayout = (responsive) => 
        <Paper sx={styles.wrapper}>
            <div style={styles.seperatorContent}>
                <img alt='super' width = "100%" height="100%" src = {seperatorImg1}/>
            </div>
            { responsive === 'desktop' ?
                <Box style={styles.container}>
                    <div style = {{ flex: 1 }}>
                        <img alt='super' width="100%" src={`data:image/png;base64, ${portraitLynn}`}/>
                    </div>
                    ...
                    <div style = {{ flex: 1, transform: 'rotateY(180deg)' }}>
                        <img alt='super' width="100%" src={`data:image/png;base64, ${portraitGranetrans}`}/>
                    </div>
                </Box>
                 :
                 <Box style = {{ textAlign: 'center' }}>
                     <div style = {styles.tabletPortraitContent}>
                        <img alt='super' width="100%" height="100%" src={`data:image/png;base64, ${portraitLynn} `}/>
                     </div>
                     <div style = {styles.titleContent }>
                    ...
                    listArray.map(item =>
                        <Box key = {item.id} sx={responsive === 'tablet' ? styles.tabletListWrapper: styles.listWrapper}>
                            <div style = {styles.listImgContent}>
                                <img alt='super' width="100%" height="100%" src={`data:image/png;base64, ${item.img}`}/>
                            </div>
